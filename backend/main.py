...
def sanitize_model_filename(filename: str, url: str = "") -> str:
    raw = str(filename or "").strip() or infer_filename_from_url(url)
    name = Path(raw).name
    if not name:
...
def model_download_paths(filename: str) -> tuple[Path, Path]:
    gguf_path = Path(os.path.expanduser(settings["gguf_directory"]))
...
def chown_like_parent(path: Path) -> None:
    try:
...
def run_model_download(task_id: str) -> None:
    with download_lock:
        task = active_downloads.get(task_id)
    if task is None:
        return

    final_path, part_path = model_download_paths(task.filename)
...
def start_download_worker(task: DownloadTask) -> None:
    worker = threading.Thread(target=run_model_download, args=(task.task_id,), daemon=True)
    task.worker = worker
    worker.start()
...
def download_model(url: str, filename: str) -> str:
    ...
    final_filename = sanitize_model_filename(filename, url)
    final_path, _ = model_download_paths(final_filename)
...
def api_list_model_downloads():
    ...
    return {"downloads": [serialize_download_task(task) for task in tasks]}
...
def api_pause_model_download(task_id: str):
    with download_lock:
        ...
        if task.status not in {"queued", "downloading"}:
            return serialize_download_task(task)
...
def api_resume_model_download(task_id: str):
    ...
    with download_lock:
        ...
        task.updated_at = datetime.now().isoformat()
        should_start = True
        payload = serialize_download_task(task)
    if should_start:
        start_download_worker(task)
    return payload
...
def api_remove_model_download(task_id: str):
    ...
    _, part_path = model_download_paths(task.filename)
    try:
        part_path.unlink(missing_ok=True)
    except Exception:
...
async def websocket_download(websocket: WebSocket, task_id: str):
    ...
    try:
        while True:
            with download_lock:
                task = active_downloads.get(task_id)
                payload = serialize_download_task(task) if task else None
            if payload is None:
